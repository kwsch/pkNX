...
{
    public static string GetTable<T1, T2>(string path) where T1 : class, IFlatBufferArchive<T2>, new() where T2 : class
    {
        var data = File.ReadAllBytes(path);
        return GetTable<T1, T2>(data);
    }

    public static string GetTable<T1, T2>(byte[] data) where T1 : class, IFlatBufferArchive<T2>, new() where T2 : class
    {
